...
function updateAll(ballot, code, firsttime) {
    ...
    if ((ballot != curBallot) || (code != curCode)) {
        ...
        var nhtxt = "2015 " + ((ballot == "N") ? "National" : "provincial legislature") + " election — " + placeinfo[code].name;
        if (firsttime) {
            hdg.text(nhtxt)
        ...
        d3.select('title').text("2015 " + ((ballot == "N") ? "National " : "provincial legislature") + " election — " + placeinfo[code].name);
        d3.select("#switchlink")
        ...
        fadetables.transition().duration(transDuration/2)
            ...
            .each("end", function() {
                ...
                newtr.append("td").attr("class", "partylogo")
                    .append("img").attr("src", function(d) { return "images/" + d.party + ".jpg"; });
